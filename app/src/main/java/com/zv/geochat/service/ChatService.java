...
public class ChatService extends Service {
    ...
    private int currentSessionMessageCount = 1;
    ...
    private void handleData(Bundle data) {
        ...
        if (command == CMD_JOIN_CHAT) {
            ...
            //Added
            getMessageLimitFromPreferences();
            if (currentSessionMessageCount < msgLimit) {
            ...
            //Added
                ...
                sendBroadcastMessageLimit(msgLimit);

                Log.d(TAG,"Limit exceeded");
            }
    ...
    private void loadUserNameFromPreferences() {
        ...
        myName = prefs.getString(Constants.KEY_USER_NAME, "Default Name");
    }

    //Added
    private void getMessageLimitFromPreferences() {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
        msgLimit = Integer.valueOf(prefs.getString(Constants.PREF_MESSAGE_LIMIT_KEY, "2"));
    }
    private void sendBroadcastMessageLimit(int msgLimit) {
        Log.d(TAG, "->(+)<- sending broadcast: BROADCAST_MSG_LIMIT");
        Intent intent = new Intent();
        intent.setAction(Constants.BROADCAST_MSG_LMT);

        Bundle data = new Bundle();
        data.putInt(Constants.CHAT_LIMIT, msgLimit);
